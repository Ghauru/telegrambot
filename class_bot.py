...
import config
from queue_module import my_queue, process_requests

storage = MemoryStorage()
sqlite_connect = sqlite3.connect('users.db', check_same_thread=False)


async def add_to_queue(text, user_id):
    await my_queue.put((text, user_id))
...
class Bot:
    def __init__(self, token):
        ...
        async def bot_message(message: types.Message):
            ...
            await self.send_message(message.chat.id, f"От какой цены начать поиск? Отправьте <b>Нет</b>,"
                                                     ...
                                                     f" Учтите, что без указания цены поиск может быть неточным",
                                    parse_mode='html')
    ...
    async def start(self):
        await executor.start_polling(dispatcher=self.dp, skip_updates=True)
